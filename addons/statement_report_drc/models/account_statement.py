...
class AccountStatement(models.TransientModel):
    ...
    @api.model
    def _get_selection_customer_supplier(self):
        if self.env.context:
            partners = self.env['res.partner'].browse(self.env.context.get('active_ids', []))
            for rec in partners:
                if rec.customer == True and rec.supplier == False:
                    return 'cust'
                elif rec.customer == False and rec.supplier == True:
                    return 'supp'
                elif rec.customer == True and rec.supplier == True:
                    continue
    ...
    radio_hide_bool = fields.Boolean(string="Hide Bool",default=_get_bool_value)
    new_radio = fields.Selection([('cust', 'Customer'), ('supp', 'Supplier'), ('cust&supp', 'Customer & Supplier')], default=_get_selection_customer_supplier, string="Selection")
    ...
    def print_report(self):
        ...
        return self.env['report'].get_action(self, 'statement_report_drc.account_statement_report', data=data)


class AccountStatementReport(models.AbstractModel):
    _name = 'report.statement_report_drc.account_statement_report'
    ...
    def _get_account_move_entry(self, accounts, sortby, display_account, partner_id, posted, account_type, currency_id, date_from, date_to):
        ...
        if sortby == 'sort_date':
            ...
            if account_res == []:
                closing = real_opening
            else:
                closing = side_balance

        return account_res, real_opening, closing
    ...
    def render_html(self, docids, data=None):
        ...
        for partner in partners:
            ...
            accounts_res, opening, closing = self.with_context(data['form'].get('used_context',{}))._get_account_move_entry(accounts, sortby, display_account, [partner.id], posted, account_type, data['form']['currency_id'], data['form']['date_from'], data['form']['date_to'])
            final_dict.update({'lines': accounts_res, 'opening': opening, 'closing': closing, 'partner': partner, 'currency': self.env['res.currency'].search([('id', '=', data['form']['currency_id'])]).name})
        ...
        return self.env['report'].render('statement_report_drc.account_statement_report', docargs)
