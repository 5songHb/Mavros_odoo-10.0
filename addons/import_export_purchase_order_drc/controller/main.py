...
class Binary(http.Controller):
    ...
    @http.route('/web/binary/download_document/PO/<model("purchase.order.import.export.wizard"):order_line>', type='http', auth="user")
    @serialize_exception
    def download_document(self, order_line, **kw):
        res = order_line.print_report_excel_data()
        data = res and res[0] or ''
        filename = len(res)==2 and res[1] or 'Purchase Order Lines'
