...
class StockWizard(models.Model):
    _name = "stock.import.export.wizard"

    choose_file = fields.Binary('Choose File')
    filename = fields.Char('filename')
    active_id = fields.Many2one('stock.picking', default=lambda self: self._context.get('active_id'))
    ...
    def validate_file(self):
        ...
        if not message:
            ...
            return {
                    'name': _(u'Import/Export Stock Lines'),
                    ...
                    if ('ID' not in data_cols) or ('ID' in data_cols and data_row[data_cols.index('ID')] == ''):
                        if str(data_row[data_cols.index('CODE')]):
                            ...
                            if product:
                                ...
                                #     default_price = True
                                vals.update({'picking_id': self.active_id.id,
                                'product_id': product.id,
                                'location_id': self.active_id.location_id.id,
                                'location_dest_id': self.active_id.location_dest_id.id,
                        ...
                        order_line = self.env['stock.move'].search([('id', '=', int(data_row[0]))])
                        if order_line:
    ...
    def print_report_excel_data(self):
        ...
        sheet_name = 'Stock Line'
        workbook = xlwt.Workbook()
        sheet = workbook.add_sheet('Stock Line 1')
        ...
        # lines = []
        for line in self.active_id.move_lines:
    ...
    @api.multi
    def export_stock_line(self):
        if len(self._context.get('active_ids')) != 1:
            raise UserError(_("You can export order lines of only one record."))
        return {
                'type': 'ir.actions.act_url',
                'url': '/web/binary/download_document/ST/%s' % self.id,
                'target': 'self'
                }
