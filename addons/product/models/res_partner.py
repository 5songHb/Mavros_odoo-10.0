...
class Partner(models.Model):
    ...
    def _default_product_pricelist(self):
        res = self.env['product.pricelist'].search([('name','=','Public Pricelist')])
        print res
    ...
    property_product_pricelist = fields.Many2one(
        ...
        default=_default_product_pricelist,
        help="This pricelist will be used, instead of the default one, for sales to the current partner",)

    #compute='_compute_product_pricelist' Removing the default Compute to set the default


    #Commenting the compute method to run default
    # @api.multi
